...
1. **Resolve the package manager from the project itself.** Inspect the `packageManager` field in `package.json`, then the lockfile (`package-lock.json`, `pnpm-lock.yaml`, `yarn.lock`, `bun.lock`, or `bun.lockb`). If those disagree, treat the explicit `packageManager` field as authoritative and report the mismatch before changing dependencies.
